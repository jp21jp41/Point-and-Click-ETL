...
class MultiData():
    ...
    def data_submission(self, the_string, lost_point):
        print('Passed Data Submission Checkpoint')
    
    def to_treeview(self, the_string, lost_point = '00', extra_data = None):
        print(lost_point)
        ...
        if 'cat_rows' in self.data_edit:
            cat_row_nums = [x for x in np.arange(0, len(self.data_edit['cat_rows']))]
            for cat_row in cat_row_nums:
                exec('tree' + str(lost_point) + '.insert("", tk.END, text = self.data_edit["cat_rows"][cat_row].name, iid = total_count, open = False)')
                total_count += 1
        if 'cat_cols' in self.data_edit:
            cat_col_nums = [x for x in np.arange(0, len(self.data_edit['cat_cols']))]
            for cat_col in cat_col_nums:
                exec('tree' + str(lost_point) + '.insert("", tk.END, text = self.data_edit["cat_cols"][cat_col].name, iid = total_count, open = False)')
                try:
                    exec('tree' + str(lost_point) + '.move(total_count, total_count - len(cat_col_nums), 0)')
                except:
                    pass
                total_count += 1
        ...
        for item_number in data_item_set:
            ...
            try:
                exec('tree' + str(lost_point) + '.move(total_count, total_count - len(cat_col_nums), total_count - len(cat_col_nums) - len(cat_row_nums))')
            except:
                try:
                    exec('tree' + str(lost_point) + '.move(total_count, total_count - len(cat_col_nums), 0)')
                except:
                    try:
                        exec('tree' + str(lost_point) + '.move(total_count, total_count - len(cat_row_nums), 0)')
                    except:
                        pass
            total_count += 1
